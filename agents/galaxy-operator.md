...
3. **Assembly Precision Mandate:** Whenever a step touches a reference genome, honor the protocol's exact version phrasing. Quote the genome request verbatim (e.g. "the latest assembly of GRCh38"); never normalize a version-constrained request down to a bare build like `hg38`/`mm10`. Resolve the value from Galaxy's **live option list** for the consuming tool at the point you need it — enumerate with `get_tool_details(io_details=True)` + `jq` (filter by base species/build keyword, never by the word "latest"), pick the label that matches the constraint, and emit an ASSEMBLY ASSERTION block before the call. Each step's value comes from that step's own phrasing and that tool's own option list; a build's index value can differ across tool wrappers, so resolve per tool rather than forcing one value across the whole pipeline. Full procedure: `skills/galaxy-histories-and-data/references/dbkey-reference.md` and `skills/galaxy-tool-execution/references/efficient-discovery.md`.
...
3. **Resolve the assembly when you need it:** As soon as the protocol mentions a reference genome, keep the request's exact phrasing in view. Before any upload that sets `dbkey=` or any tool that consumes a built-in reference index, resolve the value against Galaxy's live option list for that tool (enumerate + `jq`), pick the label matching the constraint, and emit an ASSEMBLY ASSERTION. Never resolve from training data or from the generic fallback table when the request carries a version modifier.
...
- Before invoking any tool that consumes a `reference_genome|index` / dbkey-tied built-in reference, or before you set a `dbkey` on an upload that carries a version constraint, you **must**: (a) enumerate Galaxy's live option list for that tool via `get_tool_details(io_details=True)` + `jq`, (b) pick the option whose label satisfies the protocol's wording, and (c) emit an ASSEMBLY ASSERTION block (see `galaxy-tool-execution` SKILL step 4). No silent picks. No memory-based picks. No fallback-table picks when a version modifier is present.