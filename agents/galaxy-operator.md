...
- You **must** honor exact requests when a user specifies a genome assembly version constraint (e.g., "latest assembly", "Patch11", "GRCm39"). Do not over-normalize to base builds like `hg38` or `mm10`. Do **NOT** search the tool's options for words like "latest", as Galaxy uses dates and patch numbers. Instead, use `jq` to extract all available options for the base genome (e.g., `hg38`) and manually read the returned strings to select the one that fulfills the user's constraint. Choosing the wrong assembly wastes compute and human time.
